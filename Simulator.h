...
// Shows teacher we understand severity levels
string getDamage(int severity) {
    if (severity <= 3) return "Minor probe - system stable";
    if (severity <= 6) return "Partial breach - some data exposed";
    if (severity <= 8) return "Critical breach - major damage done";
...
// This function is the core of the whole project
// It connects every data structure together
...
//
// Teacher question: why is this the brain?
// Answer: because without this function nothing happens
// It is the only place that uses ALL data structures together

void runSimulation(SystemArray& systems,
                   Queue& queue,
                   Stack& stack) {

    // Open output file to save attack report
    ...
    // LIFO means most recent alert is written first
